...
import { updateCourtierProfileAction } from "@/lib/courtier-actions";
...
export default function CourtierOnboardingWizard({ courtier }: { courtier: CourtierProfile }) {
  const [step, setStep] = useState<Step>("entreprise");
  const [entreprise, setEntreprise] = useState(courtier.entreprise ?? courtier.nom);
  const [description, setDescription] = useState(courtier.description ?? "");
  ...
  const [telephone, setTelephone] = useState(courtier.telephone ?? "");
  const [adresse, setAdresse] = useState(courtier.adresse ?? "");
  const [codePostal, setCodePostal] = useState(courtier.codePostal ?? "");
  const [ville, setVille] = useState(courtier.ville ?? "");
  const [siteWeb, setSiteWeb] = useState(courtier.siteWeb ?? "");
  ...
  function goNext() {
    ...
    if (step === "entreprise" && !entreprise.trim()) {
      setError("Merci d'indiquer le nom de votre société de courtage.");
      return;
    }
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await updateCourtierProfileAction({}, formData);
        ...
        <div className="animate-fade-up flex flex-col gap-5">
          <div>
            ...
            <p className="mt-2 text-[14.5px] text-muted">
              Facultatif — un courtier travaille souvent à distance sur toute la Pévèle, laissez
              vide si c&apos;est votre cas.
            </p>
          </div>
      ...
      {step === "recap" ? (
        <div className="animate-fade-up flex flex-col gap-5">
          <div>
            ...
            <p className="mt-2 text-[14.5px] text-muted">
              Dernière étape après ça : faire vérifier votre numéro ORIAS depuis votre espace,
              pour apparaître dans l&apos;annuaire des courtiers.
            </p>
          </div>
          ...
          >
            {pending ? "Enregistrement…" : "Terminer et faire vérifier mon compte →"}
          </button>
        </div>
      ) : null}
