...
export default function ArtisanNav({ counts }: { counts: ArtisanNavCounts }) {
  ...
  return (
    ...
    >
      {ITEMS.map((item) => {
        const active =
          item.href === "/compte/artisan" ? pathname === item.href : pathname.startsWith(item.href);
        const count = item.key ? counts[item.key as keyof ArtisanNavCounts] : 0;
